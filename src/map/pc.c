...
int max_job_table[PC_UPPER_MAX][PC_JOB_MAX] = {
	{ 10,50,50,50,50,50,50,50,50,50,50,50,50,50,50,50,50,50,50,50,99,50,50,50,70,70,50,50,50,60,60,60,60,60,60,60,60,60,60,60,60,60,50,60,60,60,50 }, // í èÌ
	{ 10,50,50,50,50,50,50,70,70,70,70,70,70,70,70,70,70,70,70,70,99,50,50,50,70,70,50,50,50,60,60,60,60,60,60,60,60,60,60,60,60,60,50,60,60,60,50 }, // ì]ê∂
	{ 10,50,50,50,50,50,50,50,50,50,50,50,50,50,50,50,50,50,50,50,99,50,50,50,70,70,50,50,50,60,60,60,60,60,60,60,60,60,60,60,60,60,50,60,60,60,50 }, // ó{éq
};
...
static struct skill_tree_entry {
	...
	short class_level;	// çƒêUÇËéûÇÃïsê≥ñhé~Å@ÉmÉr:0 àÍéü:1 ìÒéü:2 éOéü:3
...
int pc_addspiritball(struct map_session_data *sd,int interval,int num)
{
	...
	/* êEã∆Ç™ÉÇÉìÉNÇ∆èCóÖÇÃèÍçá */
	if(sd->s_class.job == PC_JOB_MO || sd->s_class.job == PC_JOB_SR)
		max = pc_checkskill(sd,MO_CALLSPIRITS);		// ãCå˜ÇÃèKìæÉåÉxÉãÇ™ç≈ëÂêî
...
#ifdef PRE_RENEWAL
	// ìyëÆê´Ç»ÇÁatkÇ™å∏è≠Ç∑ÇÈÇΩÇﬂÉXÉeÅ[É^ÉXÇåvéZÇ≥ÇπÇÈ
...
int pc_delelementball(struct map_session_data *sd, int count, int type)
{
	...
	// ìyëÆê´Ç»ÇÁdefÇ∆atkÇ™ïœìÆÇ∑ÇÈÇΩÇﬂÉXÉeÅ[É^ÉXÇåvéZÇ≥ÇπÇÈ
	if(sd->elementball.ele == ELE_EARTH)
		status_calc_pc(sd,0);
...
int pc_exp_penalty(struct map_session_data *sd, struct map_session_data *ssd, int per, int type)
{
	...
	idx = pc_search_inventory(sd, 6413);	// êgë„ÇÌÇËÇÃåÏïÑ

	// ÉåÉfÉBÉÄÉvÉeÉBÉIÇÃÉyÉiÉãÉeÉBÇÃèÍçáÅAÉIÅ[ÉâÇ»ÇÁÇŒåªç›ÇÃéÊìæåoå±ílÇ©ÇÁç∑Çµà¯Ç≠
	...
	if(loss) {
		...
		clif_msgstringtable(sd,0x729);	// êgë„ÇÌÇËÇÃåÏïÑÇ™é©ìÆè¡îÔÇ≥ÇÍÇ‹ÇµÇΩÅB
	}

	if(ssd) {
		// PKédólÅAPKÉ}ÉbÉvÇ≈çUåÇÇ™êlä‘Ç©Ç¬é©ï™Ç≈Ç»Ç¢(GXÇ»Ç«ÇÃëŒçÙ)
...
int pc_setrestartvalue(struct map_session_data *sd,int type)
{
	...
	if(sd->special_state.restart_full_recover) {	// ÉIÉVÉäÉXÉJÅ[Éh
		sd->status.hp = sd->status.max_hp;
		sd->status.sp = sd->status.max_sp;
	} else {
		if(sd->s_class.job != PC_JOB_NV) {	// ÉmÉrÇÕä˘Ç…éÄñSíºå„Ç≈HPï‚ê≥çœÇ›
	...
	if(type&2) {
		if(!(battle_config.death_penalty_type&1))	// ÉfÉXÉyÉi
...
/*==========================================
 * saveÇ…ïKóvÇ»ÉXÉeÅ[É^ÉXèCê≥ÇçsÇ»Ç§
 *------------------------------------------
 */
int pc_makesavestatus(struct map_session_data *sd)
{
	...
	// ïûÇÃêFÇÕêFÅXïæäQÇ™ëΩÇ¢ÇÃÇ≈ï€ë∂ëŒè€Ç…ÇÕÇµÇ»Ç¢
	if(!battle_config.save_clothcolor)
		sd->status.clothes_color = 0;

	// éÄñSèÛë‘ÇæÇ¡ÇΩÇÃÇ≈hpÇ1ÅAà íuÇÉZÅ[ÉuèÍèäÇ…ïœçX
	...
	// ÉZÅ[Éuã÷é~É}ÉbÉvÇæÇ¡ÇΩÇÃÇ≈éwíËà íuÇ…à⁄ìÆ
	...
	// ÉÅÉÇÉäÉAÉãÉ_ÉìÉWÉáÉìÇ…ãèÇÈÇÃÇ≈ÅAà íuÇÉZÅ[ÉuèÍèäÇ…ïœçX
	else if(map[sd->bl.m].memorial_id) {
		memcpy(&sd->status.last_point,&sd->status.save_point,sizeof(sd->status.last_point));
	}

	// ÉAÉãÉPÉ~ÇÃòAë±ê¨å˜êîï€ë∂
	...
	// É}ÉiÅ[É|ÉCÉìÉgÇ™ÉvÉâÉXÇæÇ¡ÇΩèÍçá0Ç…
...
static int pc_calcweapontype(struct map_session_data *sd)
{
	...
	if(right != WT_FIST && left == WT_FIST)			// âEéËïêäÌÇÃÇ›
		sd->status.weapon = right;
	else if(right == WT_FIST && left != WT_FIST)		// ç∂éËïêäÌÇÃÇ›
		sd->status.weapon = left;
	else if(right == WT_DAGGER && left == WT_DAGGER)	// ëoíZåï
		sd->status.weapon = WT_DOUBLE_DD;
	else if(right == WT_1HSWORD && left == WT_1HSWORD)	// ëoï–éËåï
		sd->status.weapon = WT_DOUBLE_SS;
	else if(right == WT_1HAXE && left == WT_1HAXE)		// ëoï–éËïÄ
		sd->status.weapon = WT_DOUBLE_AA;
	else if( (right == WT_DAGGER && left == WT_1HSWORD) || (right == WT_1HSWORD && left == WT_DAGGER) )	// íZåï-ï–éËåï
		sd->status.weapon = WT_DOUBLE_DS;
	else if( (right == WT_DAGGER && left == WT_1HAXE) || (right == WT_1HAXE && left == WT_DAGGER) )		// íZåï-ï–éËïÄ
		sd->status.weapon = WT_DOUBLE_DA;
	else if( (right == WT_1HSWORD && left == WT_1HAXE) || (right == WT_1HAXE && left == WT_1HSWORD) )	// ï–éËåï-ï–éËïÄ
...
/*==========================================
 * ÉNÉâÉXIDÇ©ÇÁêEã∆ílÇÃÉrÉbÉgÇï‘Ç∑
 *------------------------------------------
 */
unsigned int pc_get_job_bit(int job)
{
	...
	// êEã∆ílÇdbÇÃêEã∆ílÇ…çáÇÌÇπÇƒê›íË
	switch(job) {
		case PC_JOB_NV:		// ÉmÅ[ÉrÉX
			bit = 0x00000001;
			break;
		case PC_JOB_SM:		// É\Å[ÉhÉ}Éì
			bit = 0x00000002;
			break;
		case PC_JOB_MG:		// É}ÉWÉVÉÉÉì
			bit = 0x00000004;
			break;
		case PC_JOB_AC:		// ÉAÅ[É`ÉÉÅ[
			bit = 0x00000008;
			break;
		case PC_JOB_AL:		// ÉAÉRÉâÉCÉg
		case PC_JOB_MB:		// ÉLÉáÉìÉVÅ[ÅiébíËÅj
			bit = 0x00000010;
			break;
		case PC_JOB_MC:		// É}Å[É`ÉÉÉìÉg
			bit = 0x00000020;
			break;
		case PC_JOB_TF:		// ÉVÅ[Ét
			bit = 0x00000040;
			break;
		case PC_JOB_KN:		// ÉiÉCÉg
		case PC_JOB_DK:		// ÉfÉXÉiÉCÉgÅiébíËÅj
		case PC_JOB_RK:		// ÉãÅ[ÉìÉiÉCÉgÅiébíËÅj
			bit = 0x00000080;
			break;
		case PC_JOB_PR:		// ÉvÉäÅ[ÉXÉg
		case PC_JOB_AB:		// ÉAÅ[ÉNÉrÉVÉáÉbÉvÅiébíËÅj
			bit = 0x00000100;
			break;
		case PC_JOB_WZ:		// ÉEÉBÉUÅ[Éh
		case PC_JOB_WL:		// ÉEÉHÅ[ÉçÉbÉNÅiébíËÅj
			bit = 0x00000200;
			break;
		case PC_JOB_BS:		// ÉuÉâÉbÉNÉXÉ~ÉX
		case PC_JOB_NC:		// ÉÅÉJÉjÉbÉNÅiébíËÅj
			bit = 0x00000400;
			break;
		case PC_JOB_HT:		// ÉnÉìÉ^Å[
		case PC_JOB_RA:		// ÉåÉìÉWÉÉÅ[ÅiébíËÅj
			bit = 0x00000800;
			break;
		case PC_JOB_AS:		// ÉAÉTÉVÉì
		case PC_JOB_GC:		// ÉMÉçÉ`ÉìÉNÉçÉXÅiébíËÅj
			bit = 0x00001000;
			break;
		case PC_JOB_CR:		// ÉNÉãÉZÉCÉ_Å[
		case PC_JOB_LG:		// ÉçÉCÉÑÉãÉKÅ[ÉhÅiébíËÅj
			bit = 0x00004000;
			break;
		case PC_JOB_MO:		// ÉÇÉìÉN
		case PC_JOB_SR:		// èCóÖÅiébíËÅj
			bit = 0x00008000;
			break;
		case PC_JOB_SA:		// ÉZÅ[ÉW
		case PC_JOB_SO:		// É\Å[ÉTÉâÅ[ÅiébíËÅj
			bit = 0x00010000;
			break;
		case PC_JOB_RG:		// ÉçÅ[ÉO
		case PC_JOB_SC:		// ÉVÉÉÉhÉEÉ`ÉFÉCÉTÅ[ÅiébíËÅj
			bit = 0x00020000;
			break;
		case PC_JOB_AM:		// ÉAÉãÉPÉ~ÉXÉg
		case PC_JOB_DA:		// É_Å[ÉNÉRÉåÉNÉ^Å[ÅiébíËÅj
		case PC_JOB_GN:		// ÉWÉFÉlÉeÉBÉbÉNÅiébíËÅj
			bit = 0x00040000;
			break;
		case PC_JOB_BA:		// ÉoÅ[Éh
		case PC_JOB_MI:		// É~ÉìÉXÉgÉåÉãÅiébíËÅj
			bit = 0x00080000;
			break;
		case PC_JOB_DC:		// É_ÉìÉTÅ[
		case PC_JOB_WA:		// ÉèÉìÉ_ÉâÅ[ÅiébíËÅj
			bit = 0x00100000;
			break;
		case PC_JOB_SNV:	// ÉXÅ[ÉpÅ[ÉmÅ[ÉrÉX
		case PC_JOB_ESNV:	// ägí£ÉXÅ[ÉpÅ[ÉmÅ[ÉrÉX
			bit = 0x00800000;
			break;
		case PC_JOB_TK:		// ÉeÉRÉìÉLÉbÉh
			bit = 0x01000000;
			break;
		case PC_JOB_SG:		// åùêπ
			bit = 0x02000000;
			break;
		case PC_JOB_SL:		// É\ÉEÉãÉäÉìÉJÅ[
			bit = 0x08000000;
			break;
		case PC_JOB_GS:		// ÉKÉìÉXÉäÉìÉKÅ[
		case PC_JOB_RL:		// ÉäÉxÉäÉIÉìÅiébíËÅj
			bit = 0x10000000;
			break;
		case PC_JOB_NJ:		// îEé“
		case PC_JOB_KG:		// âeòTÅiébíËÅj
		case PC_JOB_OB:		// ûOÅiébíËÅj
			bit = 0x20000000;
			break;
		case PC_JOB_SUM:		// ÉTÉÇÉiÅ[ÅiébíËÅj
			bit = 0x40000000;
			break;
	}
...
static int pc_check_prohibition(struct map_session_data *sd, int zone)
{
	...
	if(ban) {
		// ÉeÉåÉ|ã÷é~ãÊàÊÇ≈égópïsâ¬ÇÃÉAÉCÉeÉÄÇ»ÇÁÉÅÉbÉZÅ[ÉWèoÇ∑
		if(zone&32)
...
static int pc_isequip(struct map_session_data *sd,int n)
{
	...
	if(sd->sc.data[SC_SUPERNOVICE].timer != -1) {
		...
		{
			// àÍâûÉåÉxÉãêßå¿É`ÉFÉbÉN
			if(item->elv > 0 && sd->status.base_level < item->elv)
				return -1;
			// ì™Ç≈ÉXÉgÉäÉbÉvÇ»ÇÁé∏îs
	...
	{
		if(battle_config.casting_penalty_type == 1) {		// ïêäÌÇ∆ñÓ
			if(item->equip & LOC_RARM)
				return 0;
			if(item->equip & LOC_ARROW)
				return 0;
		} else if(battle_config.casting_penalty_type == 2) {	// å¬ï 
			...
			if(item->equip & LOC_ARROW && battle_config.casting_penalty_arrow)
				return 0;
			return 0;
		} else if(battle_config.casting_penalty_type == 3) {	// ëSÇƒ
			return 0;
		}
	}
 ...
 * charéIÇ©ÇÁëóÇÁÇÍÇƒÇ´ÇΩÉXÉeÅ[É^ÉXÇê›íË
 *------------------------------------------
 */
int pc_authok(int id,struct mmo_charstatus *st,struct registry *reg)
{
	...
	if(sd == NULL)
		return 1;
	if(sd->new_fd) {
		// ÉjèdloginèÛë‘ÇæÇ¡ÇΩÇÃÇ≈ÅAóºï˚óéÇ∑
	...
	session[sd->fd]->auth = 1;	// îFèÿèIóπÇ socket.c Ç…ì`Ç¶ÇÈ
	...
	// ãCå˜ÅEÉRÉCÉìÅEâeòTÅAûOÇÃãÖëÃèâä˙âª
	sd->spiritball.num = 0;
	sd->spiritball.old = 0;
	sd->coin.num = 0;
	sd->elementball.num = 0;
	...
	// ÉAÉJÉEÉìÉgïœêîÇÃëóêMóvãÅ
	intif_request_accountreg(sd);

	// ÉNÉGÉXÉgÉäÉXÉgÇÃëóêMóvãÅ
	intif_request_quest(sd->status.account_id,sd->status.char_id);

	// é¿ê—ÉäÉXÉgÇÃëóêMóvãÅ
	...
	sd->sc.option = sd->status.option;	// optionÇÕscÇ…à⁄ÇµÇƒégÇ§

	// ÉAÉCÉeÉÄÉ`ÉFÉbÉNÇÕïKÇ∏ÉXÉeÅ[É^ÉXàŸèÌÇÃèâä˙âªå„Ç…çsÇ§
	...
	if(pc_setpos(sd, sd->status.last_point.map, sd->status.last_point.x, sd->status.last_point.y, 0)) {
		// é∏îsÇµÇΩÇÃÇ≈ê⁄ë±ÇêÿífÇ∑ÇÈ
		clif_authfail_fd(sd->fd,0);
		session[sd->fd]->auth = 0;
		return 1;
	}

	// ÉyÉbÉgÅAÉzÉÄÅAóbï∫ÅAê∏óÏÉfÅ[É^óvãÅ
	...
	// ÉeÉRÉìÉ~ÉbÉVÉáÉìÉ^Å[ÉQÉbÉgÇÃÉXÉNÉäÉvÉgïœêîÇ©ÇÁÇÃì«Ç›èoÇµÇ∆sdÇ÷ÇÃÉZÉbÉg
	sd->tk_mission_target = pc_readglobalreg(sd,"PC_MISSION_TARGET");

	// ÉâÉìÉLÉìÉOópÉ|ÉCÉìÉgÇÃÉXÉNÉäÉvÉgïœêîÇ©ÇÁÇÃì«Ç›èoÇµÇ∆sdÇ÷ÇÃÉZÉbÉg
	ranking_readreg(sd);

	// ÉtÉ@Å[É}ÉVÅ[òAë±ê¨å˜ÉJÉEÉìÉ^ ãNìÆéû0Ç…
	...
	if(battle_config.save_hate_mob) {
		// Ç»Ç©Ç¡ÇΩèÍçáÇOÇ…Ç»ÇÈÇÃÇ≈-1 ï€ë∂Ç‡+1Ç∑ÇÈÇ±Ç∆
	...
	// ÉXÉeÅ[É^ÉXèâä˙åvéZÇ»Ç«
	status_calc_pc(sd,1);

	// ÉNÉçÅ[ÉìÉXÉLÉãÇÃèâä˙âª
	if((lv = pc_checkskill2(sd,RG_PLAGIARISM)) > 0) {
		...
		if(sd->skill_clone.id > 0 && sd->skill_clone.lv > lv)	// îOÇÃÇΩÇﬂÉåÉxÉãÉ`ÉFÉbÉN
			sd->skill_clone.lv = lv;
	} else {
		memset(&sd->skill_clone,0,sizeof(sd->skill_clone));
	}

	// ÉäÉvÉçÉfÉÖÅ[ÉXÇÃèâä˙âª
	if((lv = pc_checkskill2(sd,SC_REPRODUCE)) > 0) {
		...
		if(sd->skill_reproduce.id > 0 && sd->skill_reproduce.lv > lv)	// îOÇÃÇΩÇﬂÉåÉxÉãÉ`ÉFÉbÉN
...
/*==========================================
 * session idÇ…ñ‚ëËÇ†ÇËÇ»ÇÃÇ≈å„énññ
 *------------------------------------------
 */
int pc_authfail(int id)
{
	...
	if(sd == NULL)
		return 1;
	if(sd->new_fd) {
		// ÉjèdloginèÛë‘ÇæÇ¡ÇΩÇÃÇ≈ÅAêVÇµÇ¢ê⁄ë±ÇÃÇ›óéÇ∑
...
int pc_calc_skilltree(struct map_session_data *sd)
{
	...
	for(i=0; i<MAX_PCSKILL; i++) {
		...
		if(sd->status.skill[i].flag) {		// cardÉXÉLÉãÇ»ÇÁ
			sd->status.skill[i].lv   = (sd->status.skill[i].flag == 1)? 0: sd->status.skill[i].flag-2;	// ñ{ìñÇÃlvÇ…
			sd->status.skill[i].flag = 0;	// flagÇÕ0Ç…ÇµÇƒÇ®Ç≠
		}
	}
	if(battle_config.gm_allskill > 0 && pc_isGM(sd) >= battle_config.gm_allskill) {
		...
		for(i=210; i<291; i++)
			sd->status.skill[i].id = i;
		if(battle_config.gm_allskill_addabra) { // ÉAÉuÉâÉJÉ^ÉuÉâêÍópÉXÉLÉã
			for(i=291; i<304; i++)
				sd->status.skill[i].id = i;
		}
		// åãç•ÉXÉLÉãÇ∆ÉgÉ}ÉzÅ[ÉNèúäO
		for(i=304; i<331; i++)
			sd->status.skill[i].id = i;
		// ó{éqÉXÉLÉãèúäO
		for(i=355; i<408; i++)
	...
	if(sd->sc.data[SC_ALCHEMIST].timer != -1)
	{
...
#ifdef CLASS_MB
			if(pc_checkskill(sd,AM_TWILIGHT4) == 0)	// ÉJÅ[ÉhÉXÉLÉãàµÇ¢
			{
		...
		if(pc_checkskill(sd,AM_BERSERKPITCHER) == 0)	// ÉJÅ[ÉhÉXÉLÉãàµÇ¢
		{
	...
	if(sd->sc.data[SC_KNIGHT].timer != -1)
	{
		...
		{
			if(pc_checkskill(sd,KN_ONEHAND) == 0)	// ÉJÅ[ÉhÉXÉLÉãàµÇ¢
			{
	...
	if(sd->sc.data[SC_BLACKSMITH].timer != -1)
	{
		...
		{
			if(pc_checkskill(sd,BS_ADRENALINE2) == 0)	// ÉJÅ[ÉhÉXÉLÉãàµÇ¢
			{
	...
	if(sd->sc.data[SC_HUNTER].timer != -1)
	{
		...
		{
			if(pc_checkskill(sd,HT_POWER) == 0)	// ÉJÅ[ÉhÉXÉLÉãàµÇ¢
			{
	...
	if(sd->sc.data[SC_BARDDANCER].timer != -1)
	{
		...
		if((lv = pc_checkskill(sd,BA_WHISTLE)) > 0) {
			if(pc_checkskill(sd,DC_HUMMING) == 0)	// ÉJÅ[ÉhÉXÉLÉãàµÇ¢
			{
			...
			if(pc_checkskill(sd,BA_WHISTLE) == 0)	// ÉJÅ[ÉhÉXÉLÉãàµÇ¢
			{
		...
		if((lv = pc_checkskill(sd,BA_ASSASSINCROSS)) > 0) {
			if(pc_checkskill(sd,DC_DONTFORGETME) == 0)	// ÉJÅ[ÉhÉXÉLÉãàµÇ¢
			{
			...
			if(pc_checkskill(sd,BA_ASSASSINCROSS) == 0)	// ÉJÅ[ÉhÉXÉLÉãàµÇ¢
			{
		...
		if((lv = pc_checkskill(sd,BA_POEMBRAGI)) > 0) {
			if(pc_checkskill(sd,DC_FORTUNEKISS) == 0)	// ÉJÅ[ÉhÉXÉLÉãàµÇ¢
			{
			...
			if(pc_checkskill(sd,BA_POEMBRAGI) == 0)		// ÉJÅ[ÉhÉXÉLÉãàµÇ¢
			{
		...
		if((lv = pc_checkskill(sd,BA_APPLEIDUN)) > 0) {
			if(pc_checkskill(sd,DC_SERVICEFORYOU) == 0)	// ÉJÅ[ÉhÉXÉLÉãàµÇ¢
			{
			...
			if(pc_checkskill(sd,BA_APPLEIDUN) == 0)		// ÉJÅ[ÉhÉXÉLÉãàµÇ¢
			{
...
int pc_is50overweight(struct map_session_data *sd)
{
	...
	// é©ëRâÒïúÇµÇ»Ç≠Ç»ÇÈèdó ê›íËÇ…çáÇÌÇπÇÈ
...
int pc_is90overweight(struct map_session_data *sd)
{
	...
	// é©ëRâÒïúÇµÇ»Ç≠Ç»ÇÈèdó ê›íËÇ™90%à»è„Ç»ÇÁÇªÇÍÇ…çáÇÌÇπÇÈ
	if(rate < battle_config.natural_heal_weight_rate)
		rate = battle_config.natural_heal_weight_rate;
...
int pc_skill(struct map_session_data *sd,int id,int level,int flag)
{
	...
	if(!flag && (sd->status.skill[id].id == id || level == 0)) {	// ÉNÉGÉXÉgéÊìæÇ»ÇÁÇ±Ç±Ç≈èåèÇämîFÇµÇƒëóêMÇ∑ÇÈ
		sd->status.skill[id].lv = level;
		status_calc_pc(sd,0);
		clif_skillinfoblock(sd);
	}
	else if(sd->status.skill[id].lv < level) {	// äoÇ¶ÇÁÇÍÇÈÇ™lvÇ™è¨Ç≥Ç¢Ç»ÇÁ
		if(sd->status.skill[id].id == id) {
			sd->status.skill[id].flag = sd->status.skill[id].lv+2;	// lvÇãLâØ
		} else {
			sd->status.skill[id].id   = id;
			sd->status.skill[id].flag = 1;	// cardÉXÉLÉãÇ∆Ç∑ÇÈ
		}
		sd->status.skill[id].lv = level;
	}
...
void pc_insert_card(struct map_session_data *sd, int idx_card, int idx_equip)
{
	...
	if(idx_equip >= 0 && idx_equip < MAX_INVENTORY && sd->inventory_data[idx_card]) {
		...
		if( nameid <= 0 ||
		    ...
		    (!itemdb_isarmor(sd->inventory_data[idx_equip]->nameid) && !itemdb_isweapon(sd->inventory_data[idx_equip]->nameid)) ||	// ëïîıÇ∂Ç·Ç»Ç¢
		    sd->status.inventory[idx_equip].identify == 0 ||						// ñ¢ä”íË
		    itemdb_isspecial(sd->status.inventory[idx_equip].card[0]) ||				// êªë¢ïêäÌÅEñºëOì¸ÇËÅEÉyÉbÉg
		    (sd->inventory_data[idx_equip]->equip & ep) == 0 ||						// ëïîıå¬èäà·Ç¢
		    (itemdb_isweapon(sd->inventory_data[idx_equip]->nameid) && ep == LOC_LARM) ||					// óºéËïêäÌÇ∆èÇÉJÅ[Éh
		...
		for(i=0; i<sd->inventory_data[idx_equip]->slot; i++) {
			if(sd->status.inventory[idx_equip].card[i] == 0) {
				// ï°êîÇÃìØàÍÉJÅ[ÉhÇ™ãñâ¬Ç≥ÇÍÇƒÇ¢ÇÈÇ©É`ÉFÉbÉN
				...
				// ãÛÇ´ÉXÉçÉbÉgÇ™Ç†Ç¡ÇΩÇÃÇ≈ç∑ÇµçûÇﬁ
...
/*==========================================
 * ÉXÉLÉãÇ…ÇÊÇÈîÑÇËílèCê≥
 * sdÇ™NULLÇÃÇ∆Ç´ÇÕç≈çÇílÇï‘Ç∑
 *------------------------------------------
 */
...
/*==========================================
 * ÉAÉCÉeÉÄÇîÉÇ¡ÇΩéûÇ…ÅAêVÇµÇ¢ÉAÉCÉeÉÄóìÇégÇ§Ç©ÅA
...
static int pc_checkitemlimit(struct map_session_data *sd, int idx, unsigned int tick, unsigned int now, int first)
{
	...
	if(sd->status.inventory[idx].limit > now) {
		// Ç‹Çæéûä‘Ç…Ç»Ç¡ÇƒÇ»Ç¢ÇÃÇ≈É^ÉCÉ}Å[Çåpë±Ç∑ÇÈ
	...
	if(first) {
		...
		// ìãèÊópÇÃéËçjÇ™çÌèúÇ≥ÇÍÇÈèÍçáÇÕSC_ALL_RIDINGÇâèúÇ∑ÇÈ
		if(data->nameid == 12622)
			status_change_end(&sd->bl,SC_ALL_RIDING,-1);

		// ÉtÉHÉìÉgÉAÉCÉeÉÄÇÃçÌèúéûÇÕÉtÉHÉìÉgÇèâä˙âªÇ∑ÇÈ
...
/*==========================================
 * ÉAÉCÉeÉÄí«â¡ÅBå¬êîÇÃÇ›itemç\ë¢ëÃÇÃêîéöÇñ≥éã
 *------------------------------------------
 */
int pc_additem(struct map_session_data *sd,struct item *item_data,int amount)
{
	...
	if(!itemdb_isequip2(data)) {
		// ëïîıïiÇ≈ÇÕÇ»Ç¢ÇÃÇ≈ÅAä˘èäóLïiÇ»ÇÁå¬êîÇÃÇ›ïœâªÇ≥ÇπÇÈ
		for(i=0; i<MAX_INVENTORY; i++) {
	...
	// ëïîıïiÇ©ñ¢èäóLïiÇæÇ¡ÇΩÇÃÇ≈ãÛÇ´óìÇ÷í«â¡
...
void pc_useitem(struct map_session_data *sd, int n)
{
	...
	if(nameid <= 0 || amount <= 0 || flag <= 0) {
		...
		if(flag < 0)
			clif_msgstringtable(sd, 0x6ee);	// ÉAÉCÉeÉÄÇégópÇ≈Ç´ÇÈÉåÉxÉãÇ…íBÇµÇƒÇ¢Ç‹ÇπÇÒÅB
		return;
	}
	...
	if(item->cooldown > 0) {
		...
		for(i=0;i<MAX_ITEMCOOLDOWN;i++) {
			// ìØàÍIDÇÃidxÇ©ñ¢égópidxÇíTÇ∑
			if(sd->itemcooldown[i].nameid == nameid || sd->itemcooldown[i].nameid == 0)
				break;
			// égópçœÇ›ÇÃÇ§ÇøÉNÅ[ÉãÉ^ÉCÉÄêÿÇÍÇ™Ç†ÇÍÇŒçTÇ¶ÇÈ
			if(DIFF_TICK(sd->itemcooldown[i].time, tick) <= 0) {
				j = i;
			}
		}
		// ÉAÉCÉeÉÄÉNÅ[ÉãÉ^ÉCÉÄÉäÉXÉgÇ™Ç¢Ç¡ÇœÇ¢
		if(i == MAX_ITEMCOOLDOWN) {
			// Ç©Ç¬ÉNÅ[ÉãÉ^ÉCÉÄêÿÇÍÇ™Ç»Ç¢
			if(j < 0) {
				clif_useitemack(sd,n,0,0);
				return;
			}
			// ÉNÅ[ÉãÉ^ÉCÉÄêÿÇÍidxÇ…ç∑Çµë÷Ç¶ÇÈ
			i = j;
		}
		// ìØàÍIDÇ»ÇÁÉNÅ[ÉãÉ^ÉCÉÄÉ`ÉFÉbÉN
		if(sd->itemcooldown[i].nameid == nameid) {
			int delay = DIFF_TICK(sd->itemcooldown[i].time, tick) / 1000;
			if(delay > 0) {
				clif_useitemack(sd,n,0,0);
				clif_msgstringtable3(sd, 0x746, delay + 1);	// %d ïbå„Ç…égópÇ≈Ç´Ç‹Ç∑ÅB
				return;
			}
		}
		else {	// ñ¢égópidxÇ©ÉNÅ[ÉãÉ^ÉCÉÄidxÇ»ÇÁnameidÇì¸ÇÍë÷Ç¶ÇÈ
	...
	{
		// nonconsumeÉtÉâÉOÇ™ê^Ç»ÇÁÉAÉCÉeÉÄÇè¡îÔÇµÇ»Ç¢
		if(itemdb_isnonconsume(nameid))
		{
			clif_useitemack(sd,n,amount,1);
		}
		// äyÇµÇ¢èÛë‘Ç»ÇÁämó¶ìIÇ…ÉAÅ[ÉXÉXÉpÉCÉNÉXÉNÉçÅ[ÉãÇÃè¡îÔñ≥Çµ
...
/*==========================================
 * ÉJÅ[ÉgÉAÉCÉeÉÄí«â¡ÅBå¬êîÇÃÇ›itemç\ë¢ëÃÇÃêîéöÇñ≥éã
 *------------------------------------------
 */
...
/*==========================================
 * ÉJÅ[Égì‡ÇÃÉAÉCÉeÉÄêîämîF(å¬êîÇÃç∑ï™Çï‘Ç∑)
 *------------------------------------------
 */
...
static int pc_show_steal(struct block_list *bl,va_list ap)
{
	...
	if(!type) {
		if(item == NULL)
			msg_output(fd, msg_txt(136), sd->status.name);	// %sÇ™Unknown_ItemÇÉXÉeÉBÅ[ÉãÇµÇ‹ÇµÇΩ
		else
			msg_output(fd, msg_txt(137), sd->status.name, item->jname);	// %sÇ™%sÇÉXÉeÉBÅ[ÉãÇµÇ‹ÇµÇΩ
	} else {
		msg_output(fd, msg_txt(138), sd->status.name);	// %sÇÕèdó ÉIÅ[ÉoÅ[Ç…ÇÊÇËÉAÉCÉeÉÄÇéÊìæÇ≈Ç´Ç‹ÇπÇÒÇ≈ÇµÇΩ
	}
...
int pc_setpos(struct map_session_data *sd,const char *mapname,int x,int y,int clrtype)
{
	...
	// ç¿Ç¡ÇƒÇ¢ÇΩÇÁóßÇøè„Ç™ÇÈ
	if(pc_issit(sd)) {
		pc_setstand(sd);
		skill_sit(sd,0);
	}

	// éÄÇÒÇ≈Ç¢ÇΩÇÁóßÇøè„Ç™ÇÈ
	if(unit_isdead(&sd->bl)) {
		pc_setstand(sd);
	...
	if(m < 0) {
		...
		if(map_mapname2ipport(mapname,&ip,&port) == 0) {
			// à·Ç§É}ÉbÉvÉTÅ[ÉoÅ[Ç…äÑÇËìñÇƒÇÁÇÍÇƒÇ¢ÇÈÉ}ÉbÉvÇ…à⁄ìÆ
			if( sd->pd ) {
	...
	if(m == sd->bl.m) {
		// ìØÇ∂É}ÉbÉvÇ»ÇÃÇ≈É_ÉìÉXÉÜÉjÉbÉgà¯Ç´åpÇ¨
		sd->ud.to_x = x;
		sd->ud.to_y = y;
		skill_stop_dancing(&sd->bl, 2);	// à⁄ìÆêÊÇ…ÉÜÉjÉbÉgÇà⁄ìÆÇ∑ÇÈÇ©Ç«Ç§Ç©ÇÃîªífÇ‡Ç∑ÇÈ
	} else {
		// à·Ç§É}ÉbÉvÇ»ÇÃÇ≈É_ÉìÉXÉÜÉjÉbÉgçÌèú
		skill_stop_dancing(&sd->bl, 1);

		// ëæózÅEåéÅEêØÇÃà¿äyå¯â çÌèú
		...
		// ÉÅÉÇÉäÉAÉãÉ_ÉìÉWÉáÉìÇÃÉÜÅ[ÉUÅ[êîÉ`ÉFÉbÉN
		if(map[m].memorial_id != map[sd->bl.m].memorial_id) {
			// ÉÅÉÇÉäÉAÉãÉ_ÉìÉWÉáÉìÇÃÉÜÅ[ÉUÅ[êîí«â¡
			if(map[m].memorial_id)
				memorial_addusers(map[m].memorial_id);
			// ÉÅÉÇÉäÉAÉãÉ_ÉìÉWÉáÉìÇÃÉÜÅ[ÉUÅ[êîçÌèú
	...
	// ÉfÉBÉ{Å[ÉVÉáÉìçÌèú(å£êgëŒè€é“)
	if(sd->sc.data[SC_DEVOTION].timer != -1)
		status_change_end(&sd->bl, SC_DEVOTION, -1);
	// ÉfÉBÉ{Å[ÉVÉáÉìçÌèú(å£êgé“)
	for(i=0; i<5; i++) {
		if(sd->dev.val1[i]) {
	...
	if(sd->bl.prev != NULL) {
		if(m != sd->bl.m) {
			move_flag = 1;	// êVãKÉçÉOÉCÉìÇ≈Ç»Ç≠Çƒà·Ç§MAPÇ÷à⁄ìÆÇ»ÇÁflagÉIÉì
		}
	...
	if(map[sd->bl.m].flag.gvg) {
		// çUèÈêÌéûÇ…ÉÇÉìÉXÉ^Å[ïœêgÇâèúÇ≥ÇπÇÈÇ©
		...
		// çUèÈêÌéûÇ…ÉRÉXÉ`ÉÖÅ[ÉÄÉAÉCÉeÉÄÇâèúÇ≥ÇπÇÈÇ©
		if(battle_config.disable_costume_when_gvg) {
			for(i = EQUIP_INDEX_COSTUME_HEAD; i < EQUIP_INDEX_MAX; i++) {
				if(sd->equip_index[i] >= 0 && pc_isequip(sd,sd->equip_index[i]) <= 0)
					pc_unequipitem(sd,sd->equip_index[i],1);	// ëïîıäOÇµ
			}
		}
		if(sd->sc.data[SC_SUPPORT_HPSP].timer != -1)			// ÉTÉ|Å[ÉgñÇñ@(HPSPëùâ¡)
			status_change_end(&sd->bl, SC_SUPPORT_HPSP, -1);
		if(sd->sc.data[SC_SUPPORT_SPEED].timer != -1)			// ÉTÉ|Å[ÉgñÇñ@(à⁄ìÆë¨ìxëùâ¡)
	...
	// ÉuÉçÉbÉNìoò^Ç∆spawnÇÕ clif_parse_LoadEndAck Ç≈çsÇ§

	return 0;
}

/*==========================================
 * ÉMÉãÉhÉXÉLÉãÇÃóLå¯É`ÉFÉbÉN
 * ïœâªÇ™Ç†ÇÍÇŒÉXÉeÅ[É^ÉXÇÃçƒåvéZ
 *------------------------------------------
 */
static int pc_check_guild_skill_effective_range_sub(struct guild *g,struct map_session_data *gmsd,struct map_session_data *member)
{
	...
	if(battle_config.guild_skill_effective_range > 0) {	// ÉXÉLÉãÇìØàÍãóó£îªíËÇ∑ÇÈ
		range = battle_config.guild_skill_effective_range;
		if(dx <= range && dy <= range) {	// îÕàÕì‡
			if(member->under_the_influence_of_the_guild_skill == 0)	// å¯â äOÇæÇ¡ÇΩ
				return 1;
		} else {				// îÕàÕäO
			if(member->under_the_influence_of_the_guild_skill > 0)	// å¯â ì‡ÇæÇ¡ÇΩ
				return 1;
		}
	} else {						// ÉXÉLÉãÇå¬ï Ç…ãóó£îªíËÇ∑ÇÈ
		...
		if(min_range == 0x7fffffff) {	// å¯â îÕàÕäO
			if(member->under_the_influence_of_the_guild_skill > 0)	// ëOÇÕå¯â îÕàÕì‡ÇæÇ¡ÇΩ
				return 1;
		} else {			// å¯â îÕàÕì‡
			if(member->under_the_influence_of_the_guild_skill == 0)	// ëOÇÕîÕàÕäOÇæÇ¡ÇΩ
				return 1;
			if((min_range+1) != member->under_the_influence_of_the_guild_skill)
				// ëOÇÕîÕàÕì‡Ç≈ç°âÒãóó£Ç™ïœÇÌÇ¡ÇΩ->äÙÇ¬Ç©å¯â ïœìÆ
				return 1;
		}
	}
	return 0;
...
int pc_check_guild_skill_effective_range(struct map_session_data *sd)
{
	...
	// ÉMÉãÉhéÊìæÇ≈Ç´Ç»Ç©Ç¡ÇΩ
	if((g = guild_search(sd->status.guild_id)) == NULL)
		return 0;

	// É}ÉXÉ^Å[ê⁄ë±ÇµÇƒÇ¢Ç»Ç¢
	    ...
	    pc_checkskill(gmsd,GD_HAWKEYES) > 0 )
	{
		if(sd == gmsd) {	// é©ï™Ç™ÉMÉãÉ}ÉX
			...
			for(i = 1; i < g->max_member; i++) {
				...
				if(member == NULL)	// ÉÅÉìÉoÅ[ê⁄ë±Ç»Çµ
					continue;

				if(member->bl.m != gmsd->bl.m) {	// É}ÉbÉvÇ™à·Ç§
					if(member->under_the_influence_of_the_guild_skill > 0)	// ëOâÒÇÕå¯â îÕàÕì‡ÇæÇ¡ÇΩ
			...
			if(sd->bl.m != gmsd->bl.m) {	// É}ÉbÉvÇ™à·Ç§
				if(sd->under_the_influence_of_the_guild_skill > 0)	// ëOâÒÇÕå¯â îÕàÕì‡ÇæÇ¡ÇΩ
					status_calc_pc(sd,0);
...
int pc_randomwarp(struct map_session_data *sd,int type)
{
	...
	if(map[m].flag.noteleport)	// ÉeÉåÉ|Å[Égã÷é~
		return 0;
...
int pc_runtodir(struct map_session_data *sd)
{
	...
	for(i=0; i<AREA_SIZE; i++) {
		...
		// éüÇÃÉZÉãÇ÷ÇPï‡Ç≈à⁄ìÆâ¬î\Ç≈Ç»Ç¢ÅAÇ‡ÇµÇ≠ÇÕPC,MOB,NPCÇ™ãèÇΩÇÁ
...
/*==========================================
 * ÉXÉLÉãÇÃåüçı èäóLÇµÇƒÇ¢ÇΩèÍçáLvÇ™ï‘ÇÈ
 *------------------------------------------
 */
...
/*==========================================
 * ïêäÌïœçXÇ…ÇÊÇÈÉXÉLÉãÇÃåpë±É`ÉFÉbÉN
 *------------------------------------------
 */
static int pc_checkallowskill(struct map_session_data *sd)
{
	...
	// ïêäÌÇ™çáÇÌÇ»Ç¢Ç»ÇÁÉXÉeÅ[É^ÉXàŸèÌÇâèú
	...
	if( sd->sc.data[SC_DANCING].timer != -1 ) {
		if( !(skill_get_weapontype(sd->sc.data[SC_DANCING].val1) & mask) )
			skill_stop_dancing(&sd->bl,0);	// ââëtâèú
	}
	if( sd->sc.data[SC_SPURT].timer != -1 && (sd->weapontype1 != WT_FIST || sd->weapontype2 != WT_FIST) ) {
		status_change_end(&sd->bl,SC_SPURT,-1);	// ãÏÇØë´STR
	}
#ifndef PRE_RENEWAL
	if( sd->sc.data[SC_AURABLADE].timer != -1) {	// ÉIÅ[ÉâÉuÉåÅ[Éh
		status_change_end(&sd->bl,SC_AURABLADE,-1);
	}
	if( sd->sc.data[SC_CONCENTRATION].timer != -1) {	// ÉRÉìÉZÉìÉgÉåÅ[ÉVÉáÉì
		status_change_end(&sd->bl,SC_CONCENTRATION,-1);
	}
	if( sd->sc.data[SC_TENSIONRELAX].timer != -1) {	// ÉeÉìÉVÉáÉìÉäÉâÉbÉNÉX
		status_change_end(&sd->bl,SC_TENSIONRELAX,-1);
	}
	if( sd->sc.data[SC_OVERTHRUSTMAX].timer != -1) {	// ÉIÅ[ÉoÅ[ÉgÉâÉXÉgÉ}ÉbÉNÉX
		status_change_end(&sd->bl,SC_OVERTHRUSTMAX,-1);
	}
#endif
	if( sd->sc.data[SC_HEAT_BARREL].timer != -1) {	// ÉqÅ[ÉgÉoÉåÉã
		status_change_end(&sd->bl,SC_HEAT_BARREL,-1);
	}
	if( sd->sc.data[SC_P_ALTER].timer != -1) {	// ÉvÉâÉ`ÉiÉÄÉAÉãÉ^Å[
		status_change_end(&sd->bl,SC_P_ALTER,-1);
	}

	if(sd->status.shield <= 0) {
		if(sd->sc.data[SC_AUTOGUARD].timer != -1) {		// ÉIÅ[ÉgÉKÅ[Éh
			status_change_end(&sd->bl,SC_AUTOGUARD,-1);
		}
		if(sd->sc.data[SC_DEFENDER].timer != -1) {		// ÉfÉBÉtÉFÉìÉ_Å[
			status_change_end(&sd->bl,SC_DEFENDER,-1);
		}
		if(sd->sc.data[SC_REFLECTSHIELD].timer != -1) {		// ÉäÉtÉåÉNÉgÉVÅ[ÉãÉh
			status_change_end(&sd->bl,SC_REFLECTSHIELD,-1);
		}
		if(sd->sc.data[SC_REFLECTDAMAGE].timer != -1) {		// ÉäÉtÉåÉNÉgÉ_ÉÅÅ[ÉW
			status_change_end(&sd->bl,SC_REFLECTDAMAGE,-1);
		}
		if(sd->sc.data[SC_SHIELDSPELL_DEF].timer != -1) {		// ÉVÅ[ÉãÉhÉXÉyÉã(DEF)
			status_change_end(&sd->bl,SC_SHIELDSPELL_DEF,-1);
		}
		if(sd->sc.data[SC_SHIELDSPELL_MDEF].timer != -1) {		// ÉVÅ[ÉãÉhÉXÉyÉã(MDEF)
			status_change_end(&sd->bl,SC_SHIELDSPELL_MDEF,-1);
		}
		if(sd->sc.data[SC_SHIELDSPELL_REF].timer != -1) {		// ÉVÅ[ÉãÉhÉXÉyÉã(ê∏òB)
...
static int pc_checkbaselevelup(struct map_session_data *sd)
{
	...
	if(sd->status.base_exp >= next && next > 0) {
		...
		// ÉXÉpÉmÉrÇÕÉLÉäÉGÅAÉCÉÄÉ|ÅAÉ}ÉjÉsÅAÉOÉçÅAÉTÉtÉâÇ™Ç©Ç©ÇÈ
		if(sd->s_class.job == PC_JOB_SNV || sd->s_class.job == PC_JOB_ESNV) {
			...
			clif_misceffect(&sd->bl,7);	// ÉXÉpÉmÉrìVég
		}
			...
			clif_misceffect(&sd->bl,9);	// ÉeÉRÉìånìVég
		}
		else {
		...
		// ÉåÉxÉãÉAÉbÉvÇµÇΩÇÃÇ≈ÉpÅ[ÉeÉBÅ[èÓïÒÇçXêVÇ∑ÇÈÅiåˆïΩîÕàÕÉ`ÉFÉbÉNÅj
...
int pc_gainexp(struct map_session_data *sd, struct mob_data *md, atn_bignumber base_exp, atn_bignumber job_exp, short quest)
{
	...
	if (sd->status.guild_id > 0) {	// ÉMÉãÉhÇ…è„î[
...
atn_bignumber pc_nextbaseexp(struct map_session_data *sd)
{
	...
	switch(sd->status.class_) {
		case PC_CLASS_NV:	// ÉmÅ[ÉrÉX
		case PC_CLASS_NV_B:	// ó{éqÉmÅ[ÉrÉX
			table = 0;
			break;
		case PC_CLASS_SM:	// É\Å[ÉhÉ}Éì
		case PC_CLASS_MG:	// É}ÉWÉVÉÉÉì
		case PC_CLASS_AC:	// ÉAÅ[É`ÉÉÅ[
		case PC_CLASS_AL:	// ÉAÉRÉâÉCÉg
		case PC_CLASS_MC:	// É}Å[É`ÉÉÉìÉg
		case PC_CLASS_TF:	// ÉVÅ[Ét
		case PC_CLASS_SM_B:	// ó{éqÉ\Å[ÉhÉ}Éì
		case PC_CLASS_MG_B:	// ó{éqÉ}ÉWÉVÉÉÉì
		case PC_CLASS_AC_B:	// ó{éqÉAÅ[É`ÉÉÅ[
		case PC_CLASS_AL_B:	// ó{éqÉAÉRÉâÉCÉg
		case PC_CLASS_MC_B:	// ó{éqÉ}Å[É`ÉÉÉìÉg
		case PC_CLASS_TF_B:	// ó{éqÉVÅ[Ét
		case PC_CLASS_TK:	// ÉeÉRÉìÉLÉbÉh
		case PC_CLASS_MB:	// ÉLÉáÉìÉVÅ[
			table = 1;
			break;
		case PC_CLASS_KN:	// ÉiÉCÉg
		case PC_CLASS_PR:	// ÉvÉäÅ[ÉXÉg
		case PC_CLASS_WZ:	// ÉEÉBÉUÅ[Éh
		case PC_CLASS_BS:	// ÉuÉâÉbÉNÉXÉ~ÉX
		case PC_CLASS_HT:	// ÉnÉìÉ^Å[
		case PC_CLASS_AS:	// ÉAÉTÉVÉì
		case PC_CLASS_KN2:	// ÉiÉCÉg(ãRèÊ)
		case PC_CLASS_CR:	// ÉNÉãÉZÉCÉ_Å[
		case PC_CLASS_MO:	// ÉÇÉìÉN
		case PC_CLASS_SA:	// ÉZÅ[ÉW
		case PC_CLASS_RG:	// ÉçÅ[ÉO
		case PC_CLASS_AM:	// ÉAÉãÉPÉ~ÉXÉg
		case PC_CLASS_BA:	// ÉoÅ[Éh
		case PC_CLASS_DC:	// É_ÉìÉTÅ[
		case PC_CLASS_CR2:	// ÉNÉãÉZÉCÉ_Å[(ãRèÊ)
		case PC_CLASS_KN_B:	// ó{éqÉiÉCÉg
		case PC_CLASS_PR_B:	// ó{éqÉvÉäÅ[ÉXÉg
		case PC_CLASS_WZ_B:	// ó{éqÉEÉBÉUÅ[Éh
		case PC_CLASS_BS_B:	// ó{éqÉuÉâÉbÉNÉXÉ~ÉX
		case PC_CLASS_HT_B:	// ó{éqÉnÉìÉ^Å[
		case PC_CLASS_AS_B:	// ó{éqÉAÉTÉVÉì
		case PC_CLASS_KN2_B:	// ó{éqÉiÉCÉg(ãRèÊ)
		case PC_CLASS_CR_B:	// ó{éqÉNÉãÉZÉCÉ_Å[
		case PC_CLASS_MO_B:	// ó{éqÉÇÉìÉN
		case PC_CLASS_SA_B:	// ó{éqÉZÅ[ÉW
		case PC_CLASS_RG_B:	// ó{éqÉçÅ[ÉO
		case PC_CLASS_AM_B:	// ó{éqÉAÉãÉPÉ~ÉXÉg
		case PC_CLASS_BA_B:	// ó{éqÉoÅ[Éh
		case PC_CLASS_DC_B:	// ó{éqÉ_ÉìÉTÅ[
		case PC_CLASS_CR2_B:	// ó{éqÉNÉãÉZÉCÉ_Å[(ãRèÊ)
		case PC_CLASS_SG:	// åùêπ
		case PC_CLASS_SG2:	// åùêπ(óZçá)
		case PC_CLASS_SL:	// É\ÉEÉãÉäÉìÉJÅ[
		case PC_CLASS_DK:	// ÉfÉXÉiÉCÉg
		case PC_CLASS_DA:	// É_Å[ÉNÉRÉåÉNÉ^Å[
			table = 2;
			break;
		case PC_CLASS_SNV:	// ÉXÅ[ÉpÅ[ÉmÅ[ÉrÉX
		case PC_CLASS_GS:	// ÉKÉìÉXÉäÉìÉKÅ[
		case PC_CLASS_NJ:	// îEé“
		case PC_CLASS_SNV_B:	// ó{éqÉXÅ[ÉpÅ[ÉmÅ[ÉrÉX
			table = 3;
			break;
		case PC_CLASS_NV_H:	// ì]ê∂ÉmÅ[ÉrÉX
			table = 4;
			break;
		case PC_CLASS_SM_H:	// ì]ê∂É\Å[ÉhÉ}Éì
		case PC_CLASS_MG_H:	// ì]ê∂É}ÉWÉVÉÉÉì
		case PC_CLASS_AC_H:	// ì]ê∂ÉAÅ[É`ÉÉÅ[
		case PC_CLASS_AL_H:	// ì]ê∂ÉAÉRÉâÉCÉg
		case PC_CLASS_MC_H:	// ì]ê∂É}Å[É`ÉÉÉìÉg
		case PC_CLASS_TF_H:	// ì]ê∂ÉVÅ[Ét
			table = 5;
			break;
		case PC_CLASS_KN_H:	// ÉçÅ[ÉhÉiÉCÉg
		case PC_CLASS_PR_H:	// ÉnÉCÉvÉäÅ[ÉXÉg
		case PC_CLASS_WZ_H:	// ÉnÉCÉEÉBÉUÅ[Éh
		case PC_CLASS_BS_H:	// ÉzÉèÉCÉgÉXÉ~ÉX
		case PC_CLASS_HT_H:	// ÉXÉiÉCÉpÅ[
		case PC_CLASS_AS_H:	// ÉAÉTÉVÉìÉNÉçÉX
		case PC_CLASS_KN2_H:	// ÉçÅ[ÉhÉiÉCÉg(ãRèÊ)
		case PC_CLASS_CR_H:	// ÉpÉâÉfÉBÉì
		case PC_CLASS_MO_H:	// É`ÉÉÉìÉsÉIÉì
		case PC_CLASS_SA_H:	// ÉvÉçÉtÉFÉbÉTÅ[
		case PC_CLASS_RG_H:	// É`ÉFÉCÉTÅ[
		case PC_CLASS_AM_H:	// ÉNÉäÉGÉCÉ^Å[
		case PC_CLASS_BA_H:	// ÉNÉâÉEÉì
		case PC_CLASS_DC_H:	// ÉWÉvÉVÅ[
		case PC_CLASS_CR2_H:	// ÉpÉâÉfÉBÉì(ãRèÊ)
			table = 6;
			break;
		case PC_CLASS_RK:	// ÉãÅ[ÉìÉiÉCÉg
		case PC_CLASS_WL:	// ÉEÉHÅ[ÉçÉbÉN
		case PC_CLASS_RA:	// ÉåÉìÉWÉÉÅ[
		case PC_CLASS_AB:	// ÉAÅ[ÉNÉrÉVÉáÉbÉv
		case PC_CLASS_NC:	// ÉÅÉJÉjÉbÉN
		case PC_CLASS_GC:	// ÉMÉçÉ`ÉìÉNÉçÉX
		case PC_CLASS_LG:	// ÉçÉCÉÑÉãÉKÅ[Éh
		case PC_CLASS_SO:	// É\Å[ÉTÉâÅ[
		case PC_CLASS_MI:	// É~ÉìÉXÉgÉåÉã
		case PC_CLASS_WA:	// ÉèÉìÉ_ÉâÅ[
		case PC_CLASS_SR:	// èCóÖ
		case PC_CLASS_GN:	// ÉWÉFÉlÉeÉBÉbÉN
		case PC_CLASS_SC:	// ÉVÉÉÉhÉEÉ`ÉFÉCÉTÅ[
		case PC_CLASS_RK2:	// ÉãÅ[ÉìÉiÉCÉg(ãRèÊ)
		case PC_CLASS_LG2:	// ÉçÉCÉÑÉãÉKÅ[Éh(ãRèÊ)
		case PC_CLASS_RA2:	// ÉåÉìÉWÉÉÅ[(ãRèÊ)
		case PC_CLASS_NC2:	// ÉÅÉJÉjÉbÉN(ãRèÊ)
		case PC_CLASS_RK3:	// ÉãÅ[ÉìÉiÉCÉg(ãRèÊ)
		case PC_CLASS_RK4:	// ÉãÅ[ÉìÉiÉCÉg(ãRèÊ)
		case PC_CLASS_RK5:	// ÉãÅ[ÉìÉiÉCÉg(ãRèÊ)
		case PC_CLASS_RK6:	// ÉãÅ[ÉìÉiÉCÉg(ãRèÊ)
		case PC_CLASS_RK_B:	// ó{éqÉãÅ[ÉìÉiÉCÉg
		case PC_CLASS_WL_B:	// ó{éqÉEÉHÅ[ÉçÉbÉN
		case PC_CLASS_RA_B:	// ó{éqÉåÉìÉWÉÉÅ[
		case PC_CLASS_AB_B:	// ó{éqÉAÅ[ÉNÉrÉVÉáÉbÉv
		case PC_CLASS_NC_B:	// ó{éqÉÅÉJÉjÉbÉN
		case PC_CLASS_GC_B:	// ó{éqÉMÉçÉ`ÉìÉNÉçÉX
		case PC_CLASS_LG_B:	// ó{éqÉçÉCÉÑÉãÉKÅ[Éh
		case PC_CLASS_SO_B:	// ó{éqÉ\Å[ÉTÉâÅ[
		case PC_CLASS_MI_B:	// ó{éqÉ~ÉìÉXÉgÉåÉã
		case PC_CLASS_WA_B:	// ó{éqÉèÉìÉ_ÉâÅ[
		case PC_CLASS_SR_B:	// ó{éqèCóÖ
		case PC_CLASS_GN_B:	// ó{éqÉWÉFÉlÉeÉBÉbÉN
		case PC_CLASS_SC_B:	// ó{éqÉVÉÉÉhÉEÉ`ÉFÉCÉTÅ[
		case PC_CLASS_RK2_B:	// ó{éqÉãÅ[ÉìÉiÉCÉg(ãRèÊ)
		case PC_CLASS_LG2_B:	// ó{éqÉçÉCÉÑÉãÉKÅ[Éh(ãRèÊ)
		case PC_CLASS_RA2_B:	// ó{éqÉåÉìÉWÉÉÅ[(ãRèÊ)
		case PC_CLASS_NC2_B:	// ó{éqÉÅÉJÉjÉbÉN(ãRèÊ)
		case PC_CLASS_KG:	// âeòT
		case PC_CLASS_OB:	// ûO
		case PC_CLASS_RL:	// ÉäÉxÉäÉIÉì
			table = 7;
			break;
		case PC_CLASS_RK_H:	// ì]ê∂ÉãÅ[ÉìÉiÉCÉg
		case PC_CLASS_WL_H:	// ì]ê∂ÉEÉHÅ[ÉçÉbÉN
		case PC_CLASS_RA_H:	// ì]ê∂ÉåÉìÉWÉÉÅ[
		case PC_CLASS_AB_H:	// ì]ê∂ÉAÅ[ÉNÉrÉVÉáÉbÉv
		case PC_CLASS_NC_H:	// ì]ê∂ÉÅÉJÉjÉbÉN
		case PC_CLASS_GC_H:	// ì]ê∂ÉMÉçÉ`ÉìÉNÉçÉX
		case PC_CLASS_LG_H:	// ì]ê∂ÉçÉCÉÑÉãÉKÅ[Éh
		case PC_CLASS_SO_H:	// ì]ê∂É\Å[ÉTÉâÅ[
		case PC_CLASS_MI_H:	// ì]ê∂É~ÉìÉXÉgÉåÉã
		case PC_CLASS_WA_H:	// ì]ê∂ÉèÉìÉ_ÉâÅ[
		case PC_CLASS_SR_H:	// ì]ê∂èCóÖ
		case PC_CLASS_GN_H:	// ì]ê∂ÉWÉFÉlÉeÉBÉbÉN
		case PC_CLASS_SC_H:	// ì]ê∂ÉVÉÉÉhÉEÉ`ÉFÉCÉTÅ[
		case PC_CLASS_RK2_H:	// ì]ê∂ÉãÅ[ÉìÉiÉCÉg(ãRèÊ)
		case PC_CLASS_LG2_H:	// ì]ê∂ÉçÉCÉÑÉãÉKÅ[Éh(ãRèÊ)
		case PC_CLASS_RA2_H:	// ì]ê∂ÉåÉìÉWÉÉÅ[(ãRèÊ)
		case PC_CLASS_NC2_H:	// ì]ê∂ÉÅÉJÉjÉbÉN(ãRèÊ)
		case PC_CLASS_RK3_H:	// ì]ê∂ÉãÅ[ÉìÉiÉCÉg(ãRèÊ)
		case PC_CLASS_RK4_H:	// ì]ê∂ÉãÅ[ÉìÉiÉCÉg(ãRèÊ)
		case PC_CLASS_RK5_H:	// ì]ê∂ÉãÅ[ÉìÉiÉCÉg(ãRèÊ)
		case PC_CLASS_RK6_H:	// ì]ê∂ÉãÅ[ÉìÉiÉCÉg(ãRèÊ)
		case PC_CLASS_SUM:	// ÉTÉÇÉiÅ[
		case PC_CLASS_SUM_B:	// ó{éqÉTÉÇÉiÅ[
			table = 8;
			break;
		case PC_CLASS_ESNV:	// ägí£ÉXÅ[ÉpÅ[ÉmÅ[ÉrÉX
		case PC_CLASS_ESNV_B:	// ó{éqägí£ÉXÅ[ÉpÅ[ÉmÅ[ÉrÉX
			table = 9;
			break;
		default:	// ÇªÇÍà»äO
			table = 1;
...
int pc_allskillup(struct map_session_data *sd,int flag)
{
	...
	for(i=0; i<MAX_PCSKILL; i++) {
		sd->status.skill[i].id = 0;
		if(sd->status.skill[i].flag) {	// cardÉXÉLÉãÇ»ÇÁÅA
			sd->status.skill[i].lv   = (sd->status.skill[i].flag == 1)? 0: sd->status.skill[i].flag-2;	// ñ{ìñÇÃlvÇ…
			sd->status.skill[i].flag = 0;	// flagÇÕ0Ç…ÇµÇƒÇ®Ç≠
		}
	}
	...
	if(battle_config.gm_allskill > 0 && pc_isGM(sd) >= battle_config.gm_allskill) {
		...
		for(i = 1; i <= MAX_PCSKILL; i++) {
			...
			if(skill_get_inf2(i)&INF2_NPC)
				continue;
			// ëæózÇ∆åéÇ∆êØÇÃà´ñÇÇÕèúäOÅiÉyÉiÉãÉeÉBÇÃâië±à√à≈Ç™Ç´Ç¬Ç¢ÇÃÇ≈Åj
			if(i == SG_DEVIL)
				continue;
			// ÉAÉuÉâÉJÉ^ÉuÉâêÍópÉXÉLÉãÇÕê›íËílÇ…ÇÊÇËéÊìæîªíË
		...
		for(i=0; (id = skill_tree[sd->s_class.upper][sd->s_class.job][i].id) > 0; i++) {
			if(id == SG_DEVIL)	// Ç±Ç±Ç≈èúäOèàóù
				continue;
			// flagÇ™Ç†ÇÈÇ»ÇÁÉNÉGÉXÉgÉXÉLÉãÇ‡éÊìæÇ∑ÇÈ
	...
	// status_calc_pc() ì‡Ç≈ÉpÉPÉbÉgëóêMÇ≥ÇÍÇ»Ç¢èÍçáÇ™Ç†ÇÈÇÃÇ≈Ç±Ç±Ç≈Ç‡Ç§àÍìxëóêMÇ∑ÇÈïKóvÇ™Ç†ÇÈ
	clif_skillinfoblock(sd);
...
int pc_damage(struct block_list *src,struct map_session_data *sd,int damage)
{
	...
	// ìGÇÃçUåÇÇéÛÇØÇÈÇ∆àÍíËämó¶Ç≈ëïîıÇ™âÛÇÍÇÈ
	if(sd->loss_equip_flag&0x1000 && damage > 0) {	// ñÇñ@Ç≈Ç‡âÛÇÍÇÈ
		for(i=0; i<EQUIP_INDEX_MAX; i++) {
	...
	// ìGÇÃçUåÇÇéÛÇØÇÈÇ∆àÍíËämó¶Ç≈ëïîıÇ™è¡ñ≈
	...
	// ÉXÉpÉmÉrÇ™HP0Ç…Ç»Ç¡ÇΩÇ∆Ç´ÅAExp99.0%à»è„Ç©Lv100èÛë‘Ç»ÇÁÇŒHPÇ™ëSâÒïúÇµÇƒã‡çÑèÛë‘Ç…Ç»ÇÈ
...
static int pc_dead(struct block_list *src,struct map_session_data *sd)
{
	...
	// ÉLÉâÅ[èÓïÒçXêVÇ®ÇÊÇ—OnPCKillÉCÉxÉìÉg
	if(ssd && ssd != sd) {
		if(battle_config.save_pckiller_type) {
			...
			{
				ssd->kill.char_id  = sd->status.char_id;		// é©ï™Ç™éEÇµÇΩëäéËÇÃÉLÉÉÉâID
				sd->kill.merderer_char_id = ssd->status.char_id;	// é©ï™ÇéEÇµÇΩëäéËÇÃÉLÉÉÉâID
	...
	if(sd->sc.data[SC_KAIZEL].timer != -1) {
		kaizel_lv = sd->sc.data[SC_KAIZEL].val1;	// ÉXÉeÅ[É^ÉXàŸèÌÇ™âèúÇ≥ÇÍÇÈëOÇ…ÉXÉLÉãLvÇï€ë∂
		if(kaizel_lv > 10)
			kaizel_lv = 10;
	}
	...
	{
		// îªÇËÇ…Ç≠Ç¢ÇÃÇ≈ÉäÉUÇÃÉGÉtÉFÉNÉg
	...
	if(pc_isgear(sd))		// ñÇì±ÉMÉAÇâèú
	...
	if(sd->status.merc_id > 0 && sd->mcd) {
		//clif_disp_onlyself(sd->fd, msg_txt(191));	// óbï∫Ç™ì¶ëñÇµÇ‹ÇµÇΩÅB
		clif_msgstringtable(sd, 0x4f5);
		merc_delete_data(sd);
	}

	unit_stop_walking(&sd->bl,0);
	unit_skillcastcancel(&sd->bl,0);	// ârè•ÇÃíÜé~
	skill_stop_dancing(&sd->bl, 0);
	clif_clearchar_area(&sd->bl,1);
	skill_unit_move(&sd->bl,gettick(),0);
	if(sd->sc.data[SC_BLADESTOP].timer != -1)		// îíênÇÕéñëOÇ…âèú
		status_change_end(&sd->bl,SC_BLADESTOP,-1);
	if(sd->sc.data[SC_CLOSECONFINE].timer != -1)	// ÉNÉçÅ[ÉYÉRÉìÉtÉ@ÉCÉìÇÕéñëOÇ…âèú
	...
	sd->status.die_counter++;	// éÄÇ…ÉJÉEÉìÉ^Å[èëÇ´çûÇ›
	status_change_release(&sd->bl,0x01);	// ÉXÉeÅ[É^ÉXàŸèÌÇâèúÇ∑ÇÈ

	pc_setdead(sd);

	if(sd->s_class.job == PC_JOB_NV) {
		if(battle_config.restart_hp_rate <= 50)		// ÉmÉrÇ≈ÉåÅ[Ég50à»â∫ÇÕîºï™âÒïú
			sd->status.hp = sd->status.max_hp / 2;
		else
			sd->status.hp = sd->status.max_hp * battle_config.restart_hp_rate / 100;
	}
	...
	if(battle_config.bone_drop == 2 || (battle_config.bone_drop == 1 && map[sd->bl.m].flag.pvp) || (battle_config.bone_drop == 3 && map[sd->bl.m].flag.pk))
	{
		...
		item_tmp.card[1]  = 0;
		*((int *)(&item_tmp.card[2])) = sd->status.char_id;	// ÉLÉÉÉâID
	...
	if(map[sd->bl.m].flag.pk) {
		...
		*/
		// ÉiÉCÉgÉÅÉAÉÇÅ[ÉhÉAÉCÉeÉÄÉhÉçÉbÉv
		...
		if(ssd && ssd != sd) {
			...
			ranking_setglobalreg(ssd,RK_PK);	// MOBÇ»Ç«çXêVâÒêîÇ™ëΩÇ¢èÍçáÇÕíËä˙ìIÇ…çXêV
			ranking_update(ssd,RK_PK);		// MOBÇ»Ç«çXêVâÒêîÇ™ëΩÇ¢èÍçáÇÕíËä˙ìIÇ…çXêV
	...
	if(map[sd->bl.m].flag.pvp) {
		...
		if(ssd) {
			...
			ranking_setglobalreg(ssd,RK_PVP);	// MOBÇ»Ç«çXêVâÒêîÇ™ëΩÇ¢èÍçáÇÕíËä˙ìIÇ…çXêV
			ranking_update(ssd,RK_PVP);		// MOBÇ»Ç«çXêVâÒêîÇ™ëΩÇ¢èÍçáÇÕíËä˙ìIÇ…çXêV

			// éÄÇÒÇæèÍçáÉ|ÉCÉìÉgÇå∏ÇÁÇ∑Ç»ÇÁ
			//if(ranking_get_point(sd,RK_PVP) > =1) {
			//	ranking_gain_point(sd,RK_PVP,-1);
			//	ranking_setglobalreg(sd,RK_PVP);	// MOBÇ»Ç«çXêVâÒêîÇ™ëΩÇ¢èÍçáÇÕíËä˙ìIÇ…çXêV
			//	ranking_update(sd,RK_PVP);		// MOBÇ»Ç«çXêVâÒêîÇ™ëΩÇ¢èÍçáÇÕíËä˙ìIÇ…çXêV
			//}
		}
		*/
	}
	...
	if((map[sd->bl.m].flag.pvp && sd->pvp_point < 0) || map[sd->bl.m].flag.gvg || map[sd->bl.m].flag.norevive) {
		...
		// ëSÇƒÇÃèàóùÇ™äÆóπÇµÇƒÇ©ÇÁÉJÉCÉ[ÉãÇ…ÇÊÇÈïúäà
		pc_setstand(sd);
		clif_skill_nodamage(&sd->bl,&sd->bl,ALL_RESURRECTION,4,1);
		// ÉIÉVÉäÉXÉJÅ[Éh
...
/*==========================================
 * ÉiÉCÉgÉÅÉAÉÇÅ[ÉhÇÃÉAÉCÉeÉÄÉhÉçÉbÉv
 *------------------------------------------
 */
static int pc_nightmare_drop(struct map_session_data *sd,short flag)
{
	...
	for(i=0; i<MAX_DROP_PER_MAP; i++) {
		...
		// ÉAÉCÉeÉÄIDÇ™0ÇÃÇ∆Ç´ÇÕññîˆÇ»ÇÃÇ≈èIóπ
		if((itemid = map[sd->bl.m].drop_list[i].drop_id) == 0)
			break;
		// É}ÉbÉvÉtÉâÉOî‘çÜÇ™çáÇÌÇ»Ç¢
		...
		for(j=0; j<MAX_INVENTORY; j++) {
			...
			// type=1,2ÇÕëïîıÉ`ÉFÉbÉNÅAtype=3Ç»ÇÁñ≥èåèÇ≈ãñâ¬
			if( (type == 1 && sd->status.inventory[j].equip) ||
			    (type == 2 && !sd->status.inventory[j].equip) )
				continue;

			// IDÇ™Ç†ÇÈèÍçáÅAIDÇ™àÍívÇµÇƒÇ¢Çƒämó¶OKÇ»ÇÁÉhÉçÉbÉvèàóùÇ÷
			if(itemid > 0 && itemid == sd->status.inventory[j].nameid && atn_rand()%10000 < per) {
				idx = j;
				break;
			}
			// IDÇ™ïâÇÃèÍçáÅAÉâÉìÉ_ÉÄÇ…ëŒè€ÇëIëÇ∑ÇÈÅiämó¶ÇÕÇ‹ÇæåvéZÇµÇ»Ç¢Åj
			if(itemid < 0 && atn_rand()%(++count) == 0) {
				idx = j;
			}
		}

		if(idx >= 0 && (itemid > 0 || atn_rand()%10000 < per)) {	// IDÇ™ïâÇÃèÍçáÇ…å¿ÇËÇ±Ç±Ç≈ämó¶åvéZÇ∑ÇÈ
			// ÉhÉçÉbÉvèàóù
...
int pc_setparam(struct map_session_data *sd,int type,int val)
{
	...
	case SP_ZENY:
		if(val <= MAX_ZENY) {
			// MAX_ZENY à»â∫Ç»ÇÁë„ì¸
			sd->status.zeny = val;
		} else {
			if(sd->status.zeny > val) {
				// Zeny Ç™å∏è≠ÇµÇƒÇ¢ÇÈÇ»ÇÁë„ì¸
				sd->status.zeny = val;
			} else if(sd->status.zeny <= MAX_ZENY) {
				// Zeny Ç™ëùâ¡ÇµÇƒÇ¢ÇƒÅAåªç›ÇÃílÇ™MAX_ZENY à»â∫Ç»ÇÁMAX_ZENY
				sd->status.zeny = MAX_ZENY;
			} else {
				// Zeny Ç™ëùâ¡ÇµÇƒÇ¢ÇƒÅAåªç›ÇÃílÇ™MAX_ZENY ÇÊÇËâ∫Ç»ÇÁëùâ¡ï™Çñ≥éã
				;
			}
		}
		break;
	...
	// paramÇæÇ™updatestatusèoóàÇ»Ç¢Ç‡ÇÃ
	case SP_CLASS:
	case SP_SEX:
	case SP_UPPER:
	case SP_PARTNER:
...
int pc_itemheal(struct map_session_data *sd,int hp,int sp)
{
	...
	if(hp > 0) {
		...
		if(sd->sc.data[SC_ISHA].timer != -1)		// ÉoÉCÉ^ÉäÉeÉBÉAÉNÉeÉBÉxÅ[ÉVÉáÉì
			hp = hp * 150 / 100;
		if(sd->sc.data[SC_CRITICALWOUND].timer != -1)
			hp = hp * (100 - sd->sc.data[SC_CRITICALWOUND].val1 * 10) / 100;
		if(sd->sc.data[SC_DEATHHURT].timer != -1)	// ÉfÉXÉnÅ[Ég
			hp = hp * (100 - sd->sc.data[SC_DEATHHURT].val2) / 100;
#ifndef PRE_RENEWAL
		if(sd->sc.data[SC_FUSION].timer != -1)	// ëæózÇ∆åéÇ∆êØÇÃóZçá
			hp = hp * 25 / 100;
#endif
	}
	if(sp > 0) {
...
/*==========================================
 * êEïœçX
 * à¯êî	job êEã∆ PC_JOB_NV <= job < PC_JOB_MAX
 *      upper í èÌ 0, ì]ê∂ 1, ó{éq 2, ÇªÇÃÇ‹Ç‹ -1
 *------------------------------------------
 */
int pc_jobchange(struct map_session_data *sd,int job, int upper)
{
	...
	// ÉTÉÇÉiÅ[Ç∆ëºêEã∆ÇÕëäå›Ç…ì]êEïsâ¬
	...
	// ÉXÉpÉmÉrÅAägí£ÉXÉpÉmÉrÇ≈ì]ê∂éwíËÇÃèÍçá
	if((job == PC_JOB_SNV || job == PC_JOB_ESNV) && upper == PC_UPPER_HIGH)
		upper = PC_UPPER_NORMAL;

	// ó{éq<->ì]ê∂ëOÇÃèÍçáJOB1Ç…ÇµÇ»Ç¢
	...
	if(battle_config.enable_upper_class == 0) {	// confÇ≈ñ≥å¯Ç…Ç»Ç¡ÇƒÇ¢ÇΩÇÁupper=0
		upper = PC_UPPER_NORMAL;
	}

	if(upper < 0)		// åªç›ì]ê∂Ç©Ç«Ç§Ç©ÇîªífÇ∑ÇÈ
	...
	if((sd->sex == SEX_FEMALE && job == PC_JOB_BA) || (sd->sex == SEX_MALE && job == PC_JOB_DC) ||
	   ...
	   sd->status.class_ == b_class)	// SEX_FEMALEÇÕÉoÅ[ÉhÇ…Ç»ÇÍÇ»Ç¢ÅASEX_MALEÇÕÉ_ÉìÉTÅ[Ç…Ç»ÇÍÇ»Ç¢
		return 1;

	if((sd->sex == SEX_FEMALE && job == PC_JOB_KG) || (sd->sex == SEX_MALE && job == PC_JOB_OB) ||
	   sd->status.class_ == b_class)	// SEX_FEMALEÇÕâeòTÇ…Ç»ÇÍÇ»Ç¢ÅASEX_MALEÇÕûOÇ…Ç»ÇÍÇ»Ç¢
		return 1;
...
int pc_changelook(struct map_session_data *sd,int type,int val)
{
	...
	case LOOK_CLOTHES_COLOR:
		if(val < 0 || val >= MAX_CLOTH_COLOR)
			return 0;
		if(sd->sc.data[SC_BUNSINJYUTSU].timer != -1)	// âeï™êgíÜÇÕã≠êßìIÇ…0
			val = 0;
...
void pc_setoption(struct map_session_data *sd, unsigned int type)
{
	...
	if( (type&OPTION_CARTMASK) && !pc_iscarton(sd) ) {
		...
		// ÉIÉvÉVÉáÉìílÇ©ÇÁÉJÅ[ÉgÉ^ÉCÉvÇïœä∑Ç∑ÇÈ
		switch(type) {
...
void pc_setcart(struct map_session_data *sd, unsigned short type)
{
	...
	{
#if PACKETVER < 20120201
		// å√Ç¢ÉNÉâÉCÉAÉìÉgÇ≈ÇÕÉIÉvÉVÉáÉìílÇ≈ä«óù
		{ OPTION_NOTHING,  0 },
		{ OPTION_PUSHCART,  0 },
		{ OPTION_PUSHCART2, 40 },
		{ OPTION_PUSHCART3, 65 },
		...
		{ OPTION_PUSHCART5, 90 },
#else
		// êVÇµÇ¢ÉNÉâÉCÉAÉìÉgÇ≈ÇÕèÛë‘àŸèÌÇÃval1Ç≈ä«óù
		{ CART_TYPE_NOTHING,  0 },
		{ CART_TYPE_NORMAL,   0 },
	...
	if(pc_checkskill(sd,MC_PUSHCART) > 0) {	// ÉvÉbÉVÉÖÉJÅ[ÉgÉXÉLÉãèäéù
...
int pc_setfalcon(struct map_session_data *sd)
{
	...
	if(pc_checkskill(sd,HT_FALCON) > 0) {	// ÉtÉ@ÉãÉRÉìÉ}ÉXÉ^ÉäÅ[ÉXÉLÉãèäéù
...
int pc_setdragon(struct map_session_data *sd, int type)
{
	...
	if(pc_checkskill(sd,RK_DRAGONTRAINING) > 0) { // ÉhÉâÉSÉìÉgÉåÅ[ÉjÉìÉOÉXÉLÉãèäéù
		if(type > 0 && type < 5)
...
int pc_setgear(struct map_session_data *sd)
{
	...
	if(pc_checkskill(sd,NC_MADOLICENCE) > 0) { // ñÇì±ÉMÉAÉâÉCÉZÉìÉXÉXÉLÉãèäéù
...
int pc_setregstr(struct map_session_data *sd, int reg, const char *str)
{
	...
	// strÇ™ìØàÍÇÃregstrîzóÒè„Ç…ë∂ç›ÇµÇƒÇ¢ÇÈâ¬î\ê´Ç™Ç†ÇÈÇÃÇ≈
	// reallocÇπÇ∏Ç…êÊÇ…strÇÉRÉsÅ[ÇµÇƒÇ©ÇÁfreeÇ∑ÇÈ
...
/*==========================================
 * scriptópÉOÉçÅ[ÉoÉãïœêîÇÃílÇì«Çﬁ
 *------------------------------------------
 */
...
/*==========================================
 * ÉOÉçÅ[ÉoÉãïœêîÇdirtyèÛë‘Ç…Ç∑ÇÈ
 *------------------------------------------
 */
...
/*==========================================
 * scriptópÉOÉçÅ[ÉoÉãïœêîÇÃílÇê›íË
 *------------------------------------------
 */
...
/*==========================================
 * ÔøΩRÔøΩXÔøΩ`ÔøΩÔøΩÔøΩ[ÔøΩÔøΩÔøΩîΩâfÔøΩÔøΩÔøΩÔøΩ
 *------------------------------------------
 */
...
void pc_equipitem(struct map_session_data *sd, int n, int pos)
{
	...
	if(flag <= 0 || !pos) {
		...
		if(flag < 0)
			clif_msgstringtable(sd, 0x6ed);	// ÉAÉCÉeÉÄÇëïîıÇ≈Ç´ÇÈÉåÉxÉãÇ…íBÇµÇƒÇ¢Ç‹ÇπÇÒÅB
		return;
	}
	if(pos == LOC_RLACCESSORY) {	// ÉAÉNÉZÉTÉäópó·äOèàóù
	...
	// ìÒìÅó¨èàóù
	if( pos == LOC_RLARM && 	// àÍâûÅAëïîıóvãÅâ”èäÇ™ìÒìÅó¨ïêäÌÇ©É`ÉFÉbÉNÇ∑ÇÈ
	    id->equip == LOC_RARM &&	// íPéËïêäÌ
	    (pc_checkskill(sd, AS_LEFT) > 0 || sd->s_class.job == PC_JOB_AS || sd->s_class.job == PC_JOB_GC ||
	     pc_checkskill(sd, KO_LEFT) > 0 || sd->s_class.job == PC_JOB_KG || sd->s_class.job == PC_JOB_OB) ) // ç∂éËèCòBóL
	{
		int tpos = 0;
	...
	if(pos == LOC_ARROW) {
		...
		clif_arrow_fail(sd,3);	// ëïîıÇ≈Ç´Ç‹ÇµÇΩ
	} else {
	...
	pc_checkallowskill(sd);	// ëïîıïiÇ≈ÉXÉLÉãÇ©âèúÇ≥ÇÍÇÈÇ©É`ÉFÉbÉN
	status_calc_pc(sd,0);
#if PACKETVER > 20080102
	clif_skillinfoblock(sd);
#endif
...
void pc_unequipitem(struct map_session_data *sd, int n, int type)
{
	...
	// ÉLÉÉÉXÉeÉBÉìÉOíÜ ÉXÉgÉäÉbÉvÇ∆îjâÛéûÇ™ï™Ç©ÇÁÇ»Ç¢ÇÃÇ≈ìñï™ñ¢é¿ëï
	//if(sd->state.casting) return 0;
	...
	if(sd->status.inventory[n].equip) {
		...
		for(i=0; i<EQUIP_INDEX_MAX; i++) {
			if(sd->status.inventory[n].equip & equip_pos[i])
			{
				...
				// ëïîıÇäOÇ∑Ç∆HP/SPÇÃÉyÉiÉãÉeÉBèàóù
		...
		sd->state.inventory_dirty = 1;
		sd->status.inventory[n].id = ++sd->inventory_sortkey;	// ÉCÉìÉxÉìÉgÉäÇ…êVãKìoò^
		sd->status.inventory[n].equip = LOC_NOTHING;
		if(!type)
			pc_checkallowskill(sd);
		if(sd->weapontype1 == WT_FIST && sd->weapontype2 == WT_FIST)
			status_enchant_elemental_end(&sd->bl,SC_SEVENWIND);  // ígÇ©Ç¢ïóà»äOÇÕïêäÌéùÇøêæÇ¶Ç≈ëÆê´ïtó^âèú
	} else {
	...
	if(hp) {
		if(sd->status.hp > hp) {
			...
			if(!battle_config.death_by_unrig_penalty) {
				// àÍíUHPÇ™0Ç…Ç»ÇËÅAÇ∑ÇÆÇ…1Ç…âÒïúÇ∑ÇÈ
				sd->status.hp = 0;
				clif_updatestatus(sd,SP_HP);
				pc_heal(sd,1,0);
			} else {
...
/*==========================================
 * ÉAÉCÉeÉÄÇÃindexî‘çÜÇãlÇﬂÇΩÇË
 * ëïîıïiÇÃëïîıâ¬î\É`ÉFÉbÉNÇçsÇ»Ç§
 *------------------------------------------
 */
int pc_checkitem(struct map_session_data *sd)
{
	...
	// ïœçXÇ™Ç†Ç¡ÇΩÇÁidèáÇ…É\Å[ÉgÇ∑ÇÈ
	if(sd->state.inventory_dirty) {
		if(sd->inventory_timer) {
			// égópä˙å¿ÇÃÇ†ÇÈÉAÉCÉeÉÄÇ»ÇÁàÍéûìIÇ…keyÇidÇ…ïœÇ¶ÇƒÇ®Ç≠
		...
		if(sd->inventory_timer) {
			// égópä˙å¿ÇÃÇ†ÇÈÉAÉCÉeÉÄÇ»ÇÁidÇÉTÅ[É`ÇµÇƒÉCÉìÉfÉbÉNÉXÇçƒê›íË
			struct linkdb_node *node = sd->inventory_timer;
			while(node) {
				for(i = 0; i < MAX_INVENTORY; i++) {
				...
				if(i >= MAX_INVENTORY) {
					// âΩåÃÇ©ÉCÉìÉfÉbÉNÉXÇïúå≥Ç≈Ç´Ç»Ç©Ç¡ÇΩÇÃÇ≈É\ÉPÉbÉgêÿífÇ∑ÇÈ
					if(battle_config.error_log)
		...
		for(i = 0; i < MAX_INVENTORY; i++) {
			...
			if(itemid > 0) {
				...
				if(!sd->inventory_data[i] || sd->inventory_data[i]->nameid != itemid) {
					// à íuÇ™ïœÇÌÇ¡ÇΩÇÃÇ≈ÉfÅ[É^ÉxÅ[ÉXÇçƒê›íË
...
/*==========================================
 * sdÇÕåãç•ÇµÇƒÇ¢ÇÈÇ©(ä˘ç•ÇÃèÍçáÇÕëäï˚ÇÃchar_idÇï‘Ç∑)
 *------------------------------------------
 */
...
/*==========================================
 * sdÇ™dstsdÇ∆åãç•(dstsdÅ®sdÇÃåãç•èàóùÇ‡ìØéûÇ…çsÇ§)
 *------------------------------------------
 */
...
int pc_adoption_sub(struct map_session_data* sd,struct map_session_data *papa,struct map_session_data *mama)
{
	...
	if(pc_check_adopt_condition(sd, papa, mama, 0)) {
		...
		// êeÇÕWE_BABYÇéÊìæÇ∑ÇÈÇΩÇﬂÇ…ÉXÉLÉãÉcÉäÅ[çƒåvéZ
...
int pc_check_adopt_condition(struct map_session_data *dstsd, struct map_session_data *sd, struct map_session_data *psd, short flag)
{
	...
	if(dstsd->s_class.upper != PC_UPPER_NORMAL || (dstsd->s_class.job >= PC_JOB_TK && dstsd->s_class.job <= PC_JOB_DA))
		return 0;
	// ÉpÅ[ÉeÉBÅ[ìØÇ∂É}ÉbÉvÇ…ÇRêl
	if(party_check_same_map_member_count(dstsd) != 2)
		return 0;

	if(flag) {	// ó{éqÉÅÉjÉÖÅ[Ç©ÇÁÇÃèÍçáÇÕè⁄ç◊Ç…É`ÉFÉbÉN
		int itemid = 0;
...
int pc_break_adoption(struct map_session_data *sd)
{
	...
	// %sÇ≥ÇÒÇÃóvñ]Ç…ÇÊÇËÅAó{éqä÷åWÇ™îjä¸Ç≥ÇÍÇ‹ÇµÇΩ
	...
	// âëÃèàóùÇÃé¿çsÅAå©Ç¬Ç©ÇÁÇ»ÇØÇÍÇŒcharéIÇ…àÀóä
	if(baby) {		// éqãüÇÃó£âè
	...
	if(p1) {		// êeÇÃó£âè
		p1->status.baby_id = 0;
		status_calc_pc(p1,0);	// WE_BABYÇîjä¸Ç∑ÇÈÇΩÇﬂÇ…ÉXÉLÉãÉcÉäÅ[çƒåvéZ
...
/*==========================================
 * sdÇ™ó£ç•(ëäéËÇÕsd->status.partner_idÇ…àÀÇÈ)
 * ëäéËÇ‡ìØéûÇ…ó£ç•ÅEåãç•éwó÷é©ìÆîçíD
 *------------------------------------------
 */
int pc_divorce(struct map_session_data *sd)
{
	...
	if((p_sd = pc_get_partner(sd)) != NULL) {
		...
		// ëäï˚Ç™å©Ç¬Ç©ÇÁÇ»Ç¢èÍçáÇÕcharéIÇ…èàóùÇàÀóäÇ∑ÇÈ
		chrif_reqdivorce(sd->status.partner_id);
		chrif_searchcharid(sd->status.partner_id);	// ñºëOÉfÅ[É^åƒÇ—èoÇµ
	}
...
int pc_break_equip(struct map_session_data *sd, int where)
{
	...
	switch(where) {
		...
		case LOC_BODY:
			...
			break;
		case LOC_LARM:
			if(sd->equip_index[EQUIP_INDEX_LARM] >= 0 && itemdb_isweapon(sd->inventory_data[sd->equip_index[EQUIP_INDEX_LARM]]->nameid))	// ç∂éËÇ™ïêäÌÇ»ÇÁ
				return 0;
...
int pc_break_equip2(struct map_session_data *sd,int where)
{
	...
	switch(where) {
		case EQUIP_INDEX_LACCESSORY: // ÉAÉNÉZÉTÉäç∂
		case EQUIP_INDEX_RACCESSORY:	// ÉAÉNÉZÉTÉäâE
		case EQUIP_INDEX_SHOES: // åC
		case EQUIP_INDEX_ROBE:	// å®
			break;
		case EQUIP_INDEX_HEAD:	// ì™â∫íi
		case EQUIP_INDEX_HEAD3:	// ì™íÜíi
		case EQUIP_INDEX_HEAD2:	// ì™è„íi
			if(sd->unbreakable_equip & LOC_HEAD2)
				return 0;
			if(sd->sc.data[SC_CP_HELM].timer != -1)
				return 0;
			break;
		case EQUIP_INDEX_BODY:	// ëÃ
			if(sd->unbreakable_equip & LOC_BODY)
				return 0;
			if(sd->sc.data[SC_CP_ARMOR].timer != -1)
				return 0;
			break;
		case EQUIP_INDEX_LARM:	// ç∂éË
			if(sd->equip_index[EQUIP_INDEX_LARM] >= 0 && itemdb_isweapon(sd->inventory_data[sd->equip_index[EQUIP_INDEX_LARM]]->nameid)) {	// ïêäÌ
			...
			break;
		case EQUIP_INDEX_RARM:	// âEéË
			if(sd->unbreakable_equip & LOC_RARM)
				return 0;
			if(sd->sc.data[SC_CP_WEAPON].timer != -1)
				return 0;
			break;
		case EQUIP_INDEX_ARROW: // ñÓ
			break;
		default:
			return -1;
	}
...
static int pc_spheal(struct map_session_data *sd)
{
	...
	if(pc_issit(sd))
		a += a;
	if(sd->sc.data[SC_MAGNIFICAT].timer != -1)	// É}ÉOÉjÉtÉBÉJÅ[Ég
		a += a;
	else if(sd->sc.data[SC_SHIELDSPELL_MDEF].timer != -1)	// ÉVÅ[ÉãÉhÉXÉyÉã
		a += a;
	if(sd->sc.data[SC_REGENERATION].timer != -1) {
		switch(sd->sc.data[SC_REGENERATION].val1)
		{
...
static int pc_natural_heal_sp(struct map_session_data *sd)
{
	...
	if((sd->s_class.job == PC_JOB_SNV || sd->s_class.job == PC_JOB_ESNV) || sd->sc.data[SC_EXPLOSIONSPIRITS].timer == -1 || sd->sc.data[SC_MONK].timer != -1)
		...
		sd->regen.sp += inc_num / 2;	// îöóÙîgìÆèÛë‘ÇÕSPé©ëRâÒïúë¨ìxÇ™50%
#endif
	if(sd->sc.data[SC_DANCING].timer != -1) {		// É_ÉìÉXèÛë‘Ç≈ÇÕSPÇ™âÒïúÇµÇ»Ç¢
		sd->regen.sp = 0;
		if(pc_checkskill(sd,WM_LESSON) == 0) {		// ÉåÉbÉXÉìèKìæçœÇ›Ç»ÇÁÉåÉbÉXÉìå¯â ÇÃÇ›óLå¯
			sd->regen.sptick = 0;
		}
	}
...
static int pc_rest_heal_sp(struct map_session_data *sd)
{
	...
	if(sd->regen.restsptick >= interval) {
		...
		if(sd->state.tk_doridori_sp) {
			...
			if(atn_rand()%10000 < battle_config.sg_angel_rate) {	// ëæózÇ∆åéÇ∆êØÇÃìVég
				int i;
				for(i = 0; i < 3; i++) {
					sd->feel_index[i] = -1;
					sd->hate_mob[i]   = -1;
...
static int pc_natural_heal_sub(struct map_session_data *sd,va_list ap)
{
	...
	if( (battle_config.natural_heal_weight_rate > 100 || sd->weight*100/sd->max_weight < battle_config.natural_heal_weight_rate) &&
	    !unit_isdead(&sd->bl) &&
	    !pc_ishiding(sd) &&
	    sd->sc.data[SC_POISON].timer == -1 &&	// ì≈èÛë‘Ç≈ÇÕHPÇ™âÒïúÇµÇ»Ç¢
	    sd->sc.data[SC_BLEED].timer == -1 &&	// èoååèÛë‘Ç≈ÇÕHPÇ™âÒïúÇµÇ»Ç¢
	    sd->sc.data[SC_TRICKDEAD].timer == -1 &&	// éÄÇÒÇæÇ”ÇËèÛë‘Ç≈ÇÕHPÇ™âÒïúÇµÇ»Ç¢
	    sd->sc.data[SC_GOSPEL].timer == -1 &&	// ÉSÉXÉyÉãèÛë‘Ç≈ÇÕHPÇ™âÒïúÇµÇ»Ç¢
	    sd->sc.data[SC_BERSERK].timer == -1 &&	// ÉoÅ[ÉTÅ[ÉNèÛë‘Ç≈ÇÕHPÇ™âÒïúÇµÇ»Ç¢
	    sd->sc.data[SC_MAGICMUSHROOM].timer == -1 &&	// É}ÉWÉbÉNÉ}ÉbÉVÉÖÉãÅ[ÉÄèÛë‘Ç≈ÇÕHPÇ™âÒïúÇµÇ»Ç¢
	    sd->sc.data[SC_PYREXIA].timer == -1 &&	// ÉpÉCÉåÉbÉNÉVÉAèÛë‘Ç≈ÇÕHPÇ™âÒïúÇµÇ»Ç¢
	    sd->sc.data[SC_LEECHEND].timer == -1 &&	// ÉäÅ[É`ÉGÉìÉhèÛë‘Ç≈ÇÕHPÇ™âÒïúÇµÇ»Ç¢
	    sd->sc.data[SC_INSPIRATION].timer == -1 &&	// ÉCÉìÉXÉsÉåÅ[ÉVÉáÉìèÛë‘ÇÕHPÇ™âÒïúÇµÇ»Ç¢
	    sd->sc.data[SC_RAISINGDRAGON].timer == -1 &&	// êˆó≥è∏ìVèÛë‘ÇÕHPÇ™âÒïúÇµÇ»Ç¢
	    sd->sc.data[SC_SATURDAY_NIGHT_FEVER].timer == -1 &&	// ÉtÉâÉCÉfÅ[ÉiÉCÉgÉtÉBÅ[ÉoÅ[èÛë‘Ç≈ÇÕHPÇ™âÒïúÇµÇ»Ç¢
	    sd->sc.data[SC_DIAMONDDUST].timer == -1 &&	//É_ÉCÉÑÉÇÉìÉhÉ_ÉXÉgèÛë‘Ç≈ÇÕHPÇ™âÒïúÇµÇ»Ç¢
	    sd->sc.data[SC_NATURAL_HEAL_STOP].timer == -1 )
	{
		pc_natural_heal_hp(sd);
		if( sd->sc.data[SC_MAXIMIZEPOWER].timer == -1 &&	// É}ÉLÉVÉ}ÉCÉYÉpÉèÅ[èÛë‘Ç≈ÇÕSPÇ™âÒïúÇµÇ»Ç¢
		    sd->sc.data[SC_EXTREMITYFIST].timer == -1 &&	// à¢èCóÖèÛë‘Ç≈ÇÕSPÇ™âÒïúÇµÇ»Ç¢
		    sd->sc.data[SC_BERSERK].timer == -1 &&		// ÉoÅ[ÉTÅ[ÉNèÛë‘Ç≈ÇÕSPÇ™âÒïúÇµÇ»Ç¢
		    sd->sc.data[SC_ISHA].timer == -1 &&		// ÉoÉCÉ^ÉäÉeÉBÉAÉNÉeÉBÉxÅ[ÉVÉáÉìèÛë‘Ç≈ÇÕSPÇ™âÒïúÇµÇ»Ç¢
		    sd->sc.data[SC_WEAPONBLOCKING].timer == -1 &&		// ÉEÉFÉ|ÉìÉuÉçÉbÉLÉìÉOèÛë‘Ç≈ÇÕSPÇ™âÒïúÇµÇ»Ç¢
		    sd->sc.data[SC_TOXIN].timer == -1 &&	// ÉgÉLÉVÉìèÛë‘Ç≈ÇÕSPÇ™âÒïúÇµÇ»Ç¢
		    sd->sc.data[SC_OBLIVIONCURSE].timer == -1 &&		// ÉIÉuÉäÉrÉIÉìÉJÅ[ÉXèÛë‘Ç≈ÇÕSPÇ™âÒïúÇµÇ»Ç¢
		    sd->sc.data[SC_ELECTRICSHOCKER].timer == -1 &&	// ÉGÉåÉNÉgÉäÉbÉNÉVÉáÉbÉJÅ[èÛë‘Ç≈ÇÕSPÇ™âÒïúÇµÇ»Ç¢
		    sd->sc.data[SC_CAMOUFLAGE].timer == -1 &&		// ÉJÉÇÉtÉâÅ[ÉWÉÖèÛë‘Ç≈ÇÕSPÇ™âÒïúÇµÇ»Ç¢
		    sd->sc.data[SC_MAGNETICFIELD].timer == -1 &&	// É}ÉOÉlÉeÉBÉbÉNÉtÉBÅ[ÉãÉhèÛë‘Ç≈ÇÕSPÇ™âÒïúÇµÇ»Ç¢
		    sd->sc.data[SC_STEALTHFIELD_USER].timer == -1 &&	// ÉXÉeÉãÉXÉtÉBÅ[ÉãÉh(égópé“)ÇÕSPÇ™âÒïúÇµÇ»Ç¢
		    sd->sc.data[SC__REPRODUCE].timer == -1 &&	// ÉäÉvÉçÉfÉÖÅ[ÉXèÛë‘ÇÕSPÇ™âÒïúÇµÇ»Ç¢
		    sd->sc.data[SC__SHADOWFORM].timer == -1 &&	// ÉVÉÉÉhÉEÉtÉHÅ[ÉÄèÛë‘ÇÕSPÇ™âÒïúÇµÇ»Ç¢
		    sd->sc.data[SC__INVISIBILITY].timer == -1 &&	// ÉCÉìÉrÉWÉrÉäÉeÉBèÛë‘ÇÕSPÇ™âÒïúÇµÇ»Ç¢
		    sd->sc.data[SC_SIRCLEOFNATURE].timer == -1 &&	// èzä¬Ç∑ÇÈé©ëRÇÃâπèÛë‘ÇÕSPÇ™âÒïúÇµÇ»Ç¢
		    sd->sc.data[SC_SUMMON_ELEM].timer == -1 &&	// ÉTÉÇÉìÉGÉåÉÅÉìÉ^ÉãèÛë‘ÇÕSPÇ™âÒïúÇµÇ»Ç¢
#ifndef PRE_RENEWAL
		    sd->sc.data[SC_FUSION].timer == -1 &&	// ëæózÇ∆åéÇ∆êØÇÃóZçáèÛë‘ÇÕSPÇ™âÒïúÇµÇ»Ç¢
#endif
		    sd->sc.data[SC_NATURAL_HEAL_STOP].timer == -1 )
...
/*==========================================
 * HP/SPé©ëRâÒïú (interval timerä÷êî)
 *------------------------------------------
 */
...
static int pc_autosave(int tid,unsigned int tick,int id,void *data)
{
	...
	if(interval <= 200)
		interval = 200;
	if(agit_flag == 1)	// GvGíÜÇÕÉCÉìÉ^Å[ÉoÉãÇí∑Ç≠éÊÇÈÇ±Ç∆Ç≈ÉâÉOÇä…òaÇ∑ÇÈ
			...
			if (level <= 0) {
				...
				for (account_id = start_range; account_id <= end_range; account_id++) {
					...
					if (dummy_gm_account == 0)
						dummy_gm_account = account_id;	// É_É~Å[ópÇÃGMÉAÉJÉEÉìÉgÇê›íË
				}
			}
		} else {
...
//
/*==========================================
 * ê›íËÉtÉ@ÉCÉãì«Ç›çûÇﬁ
 * exp.txt ïKóvåoå±íl
 * skill_tree.txt äeêEñàÇÃÉXÉLÉãÉcÉäÅ[
 * attr_fix.txt ëÆê´èCê≥ÉeÅ[ÉuÉã
 *------------------------------------------
 */
int pc_readdb(void)
{
	...
	for(m = 0; m < sizeof(filename2)/sizeof(filename2[0]); m++) {
		...
		while(fgets(line,1020,fp)) {
			...
			upper = atoi(split[0]);
			if(upper == PC_UPPER_HIGH && battle_config.enable_upper_class == 0)	// confÇ≈ñ≥å¯Ç…Ç»Ç¡ÇƒÇ¢ÇΩÇÁ
				continue;
	...
	if(battle_config.baby_copy_skilltree) {
		// ó{éqÇÃÉXÉLÉãÉcÉäÅ[Çí èÌêEÇ∆ìØàÍÇ…Ç∑ÇÈèÍçá
